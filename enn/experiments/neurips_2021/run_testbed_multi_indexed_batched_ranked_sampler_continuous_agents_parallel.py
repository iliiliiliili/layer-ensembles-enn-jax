...
flags.DEFINE_multi_integer(
    "max_num_samples", [10, 100], "Sample set size for selection. Continuous models are not limited by design."
)
...
flags.DEFINE_multi_integer("seed", [2, 6, 0, 5], "Seeds for testbed problem.")
...
flags.DEFINE_integer("processes", 5, "Processes to use for parallelism")

FLAGS = flags.FLAGS


def single_run(input_dim, data_ratio, noise_std, max_num_samples):
        ...
        for data_ratio in FLAGS.data_ratio:
            for noise_std in FLAGS.noise_std:
                for max_num_samples in FLAGS.max_num_samples:
                    ...
                    pool.apply_async(
                        single_run,
                        args=(input_dim, data_ratio, noise_std, max_num_samples),
                    )
